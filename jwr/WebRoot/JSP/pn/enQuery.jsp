...
<script type="text/javascript">
	...
	function query(){
		...
		var parms = {enname:enname,encid:encid};
		initLiger(parms);
	}
	function  initLiger(parms){
		window['g'] = 
            manager = $("#result").ligerGrid({
                columns: [ 
            { display: '名称', name: 'enname',width:'15%', align: 'center'  },
            ...
            { display: '主管机关', name: 'adminmot', width:'20%', heightAlign: 'center',render:function(item){return getMotVal(item.adminmot)==null?item.adminmot:getMotVal(item.adminmot)} }], 
                url: "/jwr/enControl/enBaseInfoAction.action",
                parms:parms,
                rownumbers: true,
                width: '70%', 
                height: '90%'
...
<body>
	...
	<div class="eui-info">
		...
		<div>请输入企业名称 或者 企业达标等级证书编号进行查询</div>
	</div>
	<form id="fm1">
		<p>
