    ...
    liger.getEditor = function (e)
    {
        ...
        return $.extend({
            create: function (container, editParm, controlOptions)
            {
                ...
                inputBody['ligertextbox' ]
                inputBody.appendTo(container);
                if (editParm.field)
                {
                    var txtInputName = field.name;
                    //修改源码增加 input 框 只读属性  增加 disable
                    var txtInputReadOnly = field.readonly;
                    var txtInputdisabled = field.disabled;
                    ...
                    if (!e.body)
                    {
                        ...
                        inputBody.attr($.extend({
                            //id: p.id,
                            readonly: txtInputReadOnly
                        }, field.attr));
                        inputBody.attr($.extend({
                            //id: p.id,
                        	disabled: txtInputdisabled
                        }, field.attr));
                        if (field.cssClass)
                        {
